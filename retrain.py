...
By default, this script will log summaries to /tmp/retrain_tmp/retrain_logs directory

Visualize the summaries with this command:

tensorboard --logdir /tmp/retrain_tmp/retrain_logs
...
python retrain.py (... other args as before ...) \
    --saved_model_dir=/tmp/retrain_tmp/saved_models/$(date +%s)/
tensorflow_model_server --port=9000 --model_name=my_image_classifier \
    --model_base_path=/tmp/retrain_tmp/saved_models/
```
"""
...
if __name__ == '__main__':
  if not os.path.isdir("/tmp/retrain_tmp"):
    os.mkdir("/tmp/retrain_tmp")
  ...
  parser.add_argument(
      '--output_graph',
      type=str,
      default='/tmp/retrain_tmp/output_graph.pb',
      help='Where to save the trained graph.'
  )
  parser.add_argument(
      '--intermediate_output_graphs_dir',
      type=str,
      default='/tmp/retrain_tmp/intermediate_graph/',
  ...
  parser.add_argument(
      '--output_labels',
      type=str,
      default='/tmp/retrain_tmp/output_labels.txt',
      help='Where to save the trained graph\'s labels.'
  )
  parser.add_argument(
      '--summaries_dir',
      type=str,
      default='/tmp/retrain_tmp/retrain_logs',
  ...
  parser.add_argument(
      '--bottleneck_dir',
      type=str,
      default='/tmp/retrain_tmp/bottleneck',
  ...
  parser.add_argument(
      '--checkpoint_path',
      type=str,
      default='/tmp/retrain_tmp/_retrain_checkpoint',
